...
(e.g.,~\cite{tang2022gait,casiez2012oneeuro,welch2006kalman,hsu2024accv, chen2021anatomy}),
...
Central to this approach is \textbf{FormScript}, a semantic intermediate representation that transforms raw pose measurements into interpretable \emph{FormCodes}, which are discrete biomechanical descriptors that enable rule-based form analysis and exercise-specific reasoning.
These FormCodes drive both real-time corrective feedback and \textbf{LLM-powered post-workout summaries} (using Llama 3.3 70B via Cerebras), which synthesize per-rep feedback into personalized coaching recommendations.
The system personalizes its critique level through a short calibration phase and produces both live corrections and comprehensive session summaries.
