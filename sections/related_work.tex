...
Recent work in computer vision has sought to bridge the gap between geometric pose data and natural language. PoseScript \cite{delmas2022posescript} introduced the concept of "posecodes"—atomic, rule-based boolean descriptors extracted from 3D keypoints—to generate synthetic textual descriptions of static human poses. Our system adopts a nearly identical architectural strategy, defining "FormCodes" as the semantic intermediate layer. However, while PoseScript focuses on static pose captioning, FitCoachAR extends this methodology to dynamic, temporal motion analysis, using these atomic codes to drive real-time coaching feedback.
