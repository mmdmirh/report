...
We further observe that 2D-to-3D lifting pipelines (HRNet+VideoPose and HRNet+MotionBERT) perform worse than HRNet alone, likely due to overfitting to motion patterns in their training datasets that differ from strength-training exercises.
In contrast, monocular 3D pose estimation models and heavier 2D-based models achieve high accuracy but suffer from prohibitive latency, making them unsuitable for real-time feedback scenarios.
...
As shown in Table~\ref{tab:movenet_ours_std},
our post-processing consistently reduces variance across viewpoints
for both joint angles and distances.
Improvements are particularly evident for side-view–sensitive joints
such as elbows and knees, indicating increased robustness to
foreshortening and partial occlusion.
